...
# Ensure all CUDA and nvidia pip libraries are in LD_LIBRARY_PATH
NVIDIA_LIBS=$("$DFL_PYTHON" -c "import site, glob; print(':'.join(glob.glob(site.getsitepackages()[0] + '/nvidia/*/lib')))" 2>/dev/null)
